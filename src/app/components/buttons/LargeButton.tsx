...
const LargeButton: React.FC<LargeButtonProps> = ({ icon, type, formStatus,isMostPopular = false, text = 'Nossos serviços' }) => {
  return (
    <button className={` flex flex-row self-stretch items-center justify-center gap-8 px-8 py-4  text-[#F4F5F7] bg-[#212740] hover:bg-[#282C41] active:bg-[#202335] active:border-0 rounded-xs font-medium cursor-pointer transition-all duration-300 text-[1.4vh] ease-in-out ${formStatus === 'sending' && 'cursor-wait  disabled:bg-[#202335]/10'} ${formStatus === 'sent' && 'disabled:bg-green-700'}`} type={type} disabled={formStatus === 'sending' || formStatus === 'sent'}>
